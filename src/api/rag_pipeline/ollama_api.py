...
from rag_pipeline.config import logger, GENERATION_CONFIG
    ...
    def _ensure_model_exists(self):
        ...
        try:
            ...
            if self.model_name not in result.stdout:
                logger.warning(f"Model {self.model_name} not found locally. Make sure it's downloaded.")
                logger.info("Available models: " + result.stdout)
        except subprocess.CalledProcessError as e:
            logger.exception(f"Error checking Ollama models: {str(e)}")
        except FileNotFoundError:
            logger.error("Ollama command not found. Make sure Ollama is installed and in your PATH.")

    def _create_temp_model(self, temperature: float = 0.7, top_p: float = 0.9,
...
""")
    ...
    def generate_text(self, prompt: str, temperature: float = 0.7, top_p: float = 0.9,
                     repeat_penalty: float = 1.1, max_tokens: int = 2048) -> str:
        """
...
def format_prompt(system_prompt: str, context: str, question: str, conversation_history: str = "") -> str:
    """
