...
async def get_query_count(
    ...
    try:
        ...
        result = db.execute(
            text("SELECT COUNT(*) FROM audit WHERE event_time > CURRENT_TIMESTAMP - INTERVAL ':days days'"),
            {"days": days}
        ).scalar()
...
async def get_top_keywords(
    ...
    exclude_words = ["what", "when", "where", "which", "who", "how", "why", 
                     "the", "and", "for", "that", "this", "with", "can", 
                     "you", "have", "are", "not", "from", "your"]
...
async def get_system_stats(user_email: str = Depends(verify_token)):
    ...
    try:
        ...
        # Total queries
        stats["total_queries"] = db.execute(
            text("SELECT COUNT(*) FROM audit")
        ).scalar()
        ...
        # Total classes
        stats["total_classes"] = db.execute(
            text("SELECT COUNT(*) FROM class")
        ).scalar()
        ...
        stats["queries_last_24h"] = db.execute(
            text("SELECT COUNT(*) FROM audit WHERE event_time > CURRENT_TIMESTAMP - INTERVAL '1 day'")
        ).scalar()
        
        # Active users in last 24 hours
        stats["active_users_last_24h"] = db.execute(
            text("SELECT COUNT(DISTINCT user_email) FROM audit WHERE event_time > CURRENT_TIMESTAMP - INTERVAL '1 day'")
        ).scalar()
