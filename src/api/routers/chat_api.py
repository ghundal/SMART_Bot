...
async def start_chat_with_llm(
    ...
    print(f"User {user_email} starting chat with content: {message.get('content')} in session: {x_session_id}")
...
async def continue_chat_with_llm(
    ...
    print(f"User {user_email} continuing chat {chat_id} with content: {message.get('content')} in session: {x_session_id}")
